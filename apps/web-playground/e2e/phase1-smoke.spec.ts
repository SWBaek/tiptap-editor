...
test("loads the Phase 3 playground and exercises preview/export basics", async ({ page }) => {
  ...
  await expect(page.getByRole("button", { name: "Documents panel" })).toHaveAttribute("aria-pressed", "false");
  ...
  await expect(filesPanel.getByRole("heading", { name: "Documents" })).toBeVisible();
  await expect(filesPanel).not.toContainText("Phase 3 Playground");
  await page.getByRole("button", { name: "Documents panel" }).click();
  await expect(page.getByRole("complementary", { name: "Documents side panel" })).toBeHidden();
  ...
  await expect(page.locator(".tabs").getByRole("button", { name: "JSON" })).toHaveCount(0);
  ...
  await page.getByRole("dialog", { name: "Export document" }).getByRole("button", { name: "Export Markdown" }).click();
...
test("shows authoring structure projections without changing heading text", async ({ page }) => {
  ...
  await page.getByRole("button", { name: "Settings panel" }).click();
  const settingsPanel = page.getByRole("complementary", { name: "Settings side panel" });
  await settingsPanel.getByRole("tab", { name: "Application" }).click();
...
test("uses the Review side panel for diff workflow controls", async ({ page }) => {
  ...
  await expect(reviewPanel.locator(".status-block").filter({ hasText: "Base" })).toContainText("Last saved version");
  ...
  await reviewPanel.getByRole("button", { name: "Show diff" }).click();
  await expect(page.locator(".diff-review-base")).toContainText("Last saved version");
...
test("keeps browser Documents focused on explicit files and downloads", async ({ page }) => {
  await page.goto("/");
  await page.getByRole("button", { name: "Documents panel" }).click();
  const filesPanel = page.getByRole("complementary", { name: "Documents side panel" });
...
test("shows a desktop start screen before opening a Tauri workspace document", async ({ page }) => {
  ...
  await page.addInitScript(() => {
    ...
    const addWorkspaceEntry = (relativePath: string, kind: "folder" | "sdoc-file") => {
      ...
      if (!name) {
        throw new Error("Missing workspace entry name");
      }
      ...
      const entry: WorkspaceEntry = {
        name,
        path: `C:\\Docs\\${relativePath.replaceAll("/", "\\")}`,
        kind,
    ...
    (window as typeof window & {
      ...
      async openSdocPath(path: string) {
        (window as typeof window & { __LAST_OPENED_PATH__?: string }).__LAST_OPENED_PATH__ = path;
      ...
      async listSdocWorkspaceEntries() {
        return structuredClone(workspaceEntries);
      },
      ...
      async createSdocWorkspaceFile(_directoryPath: string, relativePath: string, bytes: Uint8Array) {
        if (!(bytes instanceof Uint8Array) || bytes[0] !== 80 || bytes[1] !== 75) {
  ...
  await expect(filesPanel.locator(".explorer-root")).toHaveCount(0);
  ...
  await expect(filesPanel).not.toContainText("Developer workspace");
  const workspaceTree = filesPanel.getByRole("tree", { name: "Workspace folders and documents" });
  ...
  const sidebarResizeHandle = filesPanel.getByRole("separator", { name: "Resize sidebar" });
  await sidebarResizeHandle.focus();
  await sidebarResizeHandle.press("End");
  ...
  await sidebarResizeHandle.press("ArrowRight");
  await expect.poll(async () => Math.round((await sidebar.boundingBox())?.width ?? 0)).toBe(230);
  await expect.poll(() => page.evaluate(() => window.localStorage.getItem("sdoc-sidebar-width"))).toBe("230");
  await sidebarResizeHandle.dblclick();
  await expect.poll(async () => Math.round((await sidebar.boundingBox())?.width ?? 0)).toBe(260);
  await page.locator(".workspace").click({ position: { x: 4, y: 4 } });
  ...
  await expect(guidesTreeItem).toBeFocused();
  await expect(workspaceTree.locator('[role="treeitem"][tabindex="0"]')).toHaveCount(1);
  ...
  let newFolderInput = filesPanel.getByLabel("New folder name");
  await newFolderInput.press("Escape");
  await expect(newFolderInput).toHaveCount(0);
  await filesPanel.getByRole("button", { name: "Create folder" }).click();
  newFolderInput = filesPanel.getByLabel("New folder name");
  await newFolderInput.fill("API");
  await newFolderInput.press("Enter");
  await expect(page.locator(".status-note")).toContainText("Created folder Guides/API");
  await filesPanel.getByRole("button", { name: "Expand folder API" }).click();
  await filesPanel.getByRole("button", { name: "Create SDoc document" }).click();
  const newDocumentInput = filesPanel.getByLabel("New document name");
  await newDocumentInput.fill("Draft");
  await newDocumentInput.press("Enter");
  ...
  await expect(filesPanel.locator(".explorer-entry-row.active")).toContainText("Draft.sdoc");
  await filesPanel.getByRole("button", { name: "Actions for API" }).click();
  await page.getByRole("menu", { name: "Workspace actions for API" }).getByRole("menuitem", { name: "Rename" }).click();
  const renameFolderInput = filesPanel.getByLabel("Rename API");
  await renameFolderInput.fill("Reference");
  await renameFolderInput.press("Enter");
  await expect(page.locator(".status-note")).toContainText("Renamed to Guides/Reference");
  const draftTreeItem = workspaceTree.getByRole("treeitem", { name: "Draft.sdoc" });
  await draftTreeItem.focus();
  await draftTreeItem.press("F2");
  const renameDocumentInput = filesPanel.getByLabel("Rename Draft.sdoc");
  await renameDocumentInput.fill("Review");
  await renameDocumentInput.press("Enter");
  await expect(filesPanel.locator(".explorer-entry-row.active")).toContainText("Review.sdoc");
  const reviewTreeItem = workspaceTree.getByRole("treeitem", { name: "Review.sdoc" });
  await reviewTreeItem.focus();
  await reviewTreeItem.press("Shift+F10");
  const reviewActionsMenu = page.getByRole("menu", { name: "Workspace actions for Review.sdoc" });
  await expect(reviewActionsMenu).toBeVisible();
  await expect(reviewActionsMenu.getByRole("menuitem", { name: "Rename" })).toBeFocused();
  await page.keyboard.press("End");
  await expect(reviewActionsMenu.getByRole("menuitem", { name: "Move to Trash" })).toBeFocused();
  const menuBounds = await reviewActionsMenu.boundingBox();
  const viewport = page.viewportSize();
  expect(menuBounds?.x).toBeGreaterThanOrEqual(0);
  expect(menuBounds?.y).toBeGreaterThanOrEqual(0);
  expect((menuBounds?.x ?? 0) + (menuBounds?.width ?? 0)).toBeLessThanOrEqual(viewport?.width ?? 0);
  expect((menuBounds?.y ?? 0) + (menuBounds?.height ?? 0)).toBeLessThanOrEqual(viewport?.height ?? 0);
  await page.keyboard.press("Escape");
  await expect(reviewActionsMenu).toHaveCount(0);
  await expect(reviewTreeItem).toBeFocused();
  await filesPanel.locator('summary[aria-label="More Explorer actions"]').click();
  await filesPanel.getByRole("menu", { name: "More Explorer actions" }).getByRole("menuitem", { name: "Collapse All" }).click();
  await expect(filesPanel.getByRole("button", { name: "Review.sdoc", exact: true })).toHaveCount(0);
  ...
  await page.getByRole("menu", { name: "Workspace actions for Reference" }).getByRole("menuitem", { name: "Move to Trash" }).click();
  await trashDialog.getByRole("button", { name: "Move to Trash" }).click();
...
test("exports deliverables and keeps developer outputs separate", async ({ page }) => {
  ...
  await page.getByRole("button", { name: "Export", exact: true }).click();
  let exportDialog = page.getByRole("dialog", { name: "Export document" });
  await expect(exportDialog).toBeVisible();
  await expect(exportDialog.getByRole("radio", { name: /Markdown/ })).toBeChecked();
  await expect(exportDialog.getByLabel("Export destination")).toContainText("Export Panel Spec.md");
  await expect(exportDialog).not.toContainText("document.json");
  await expect(exportDialog).not.toContainText("plain.md");
  ...
  await exportDialog.getByRole("button", { name: "Export HTML" }).click();
  ...
  await page.getByRole("button", { name: "Export", exact: true }).click();
  exportDialog = page.getByRole("dialog", { name: "Export document" });
  const markdownDownload = page.waitForEvent("download");
  ...
  await expect(page.getByRole("button", { name: "Developer panel" })).toHaveCount(0);
  await page.getByRole("button", { name: "Settings panel" }).click();
  await page.getByRole("complementary", { name: "Settings side panel" }).getByRole("tab", { name: "Developer" }).click();
...
test("stores local history snapshots and compares them with the current document", async ({ page }) => {
  ...
  await historyReviewPanel.getByRole("button", { name: "Use last saved version" }).click();
  await expect(page.locator(".status-note")).toContainText("Comparing with last saved version");
  await expect(historyReviewPanel.locator(".status-block").filter({ hasText: "Base" })).toContainText("Last saved version");
...
test("resolves a Draw.io external edit conflict as a revision asset", async ({ page }, testInfo) => {
  ...
  await page.addInitScript(() => {
    ...
    (window as typeof window & { __SDOC_NATIVE_SAVE_BRIDGE__?: unknown }).__SDOC_NATIVE_SAVE_BRIDGE__ = {
      ...
      async chooseSdocSavePath() {
        return "C:\\Docs\\Drawio Conflict Revision.sdoc";
      },
  ...
  await page.goto("/");
  await page.getByRole("button", { name: "New .sdoc" }).click();
  await selectPreviewTab(page, "JSON");
  await page.getByRole("button", { name: "Settings panel" }).click();
  const settingsPanel = page.getByRole("complementary", { name: "Settings side panel" });
  await settingsPanel.getByRole("tab", { name: "Application" }).click();
  await settingsPanel.getByLabel("Draw.io application").fill("C:\\Program Files\\draw.io\\draw.io.exe");
  await page.locator(".editor-surface").click();
  ...
  await page.getByRole("button", { name: "Save .sdoc As" }).click();
  await expect.poll(() => page.evaluate(() => (window as typeof window & { __LAST_SAVED_PATH__?: string }).__LAST_SAVED_PATH__)).toBe("C:\\Docs\\Drawio Conflict Revision.sdoc");
});
...
async function selectPreviewTab(page: Page, tab: "JSON" | "Markdown" | "Diff"): Promise<void> {
  if (tab === "JSON" && (await page.locator(".tabs").getByRole("button", { name: "JSON" }).count()) === 0) {
    const settingsPanel = page.getByRole("complementary", { name: "Settings side panel" });
    const settingsWasOpen = await settingsPanel.isVisible().catch(() => false);
    const previousPanelButton = page.getByRole("navigation", { name: "Primary" }).locator('button[aria-pressed="true"]');
    const previousPanelName = !settingsWasOpen && await previousPanelButton.count() > 0
      ? await previousPanelButton.getAttribute("aria-label")
      : null;
    if (!settingsWasOpen) {
      await page.getByRole("button", { name: "Settings panel" }).click();
    }
    await settingsPanel.getByRole("tab", { name: "Developer" }).click();
    const developerToggle = settingsPanel.getByLabel("Enable developer tools");
    if (!(await developerToggle.isChecked())) {
      await developerToggle.check();
    }
    if (!settingsWasOpen) {
      await page.getByRole("button", { name: previousPanelName ?? "Settings panel" }).click();
    }
  }
