...
export function DesktopStartScreen({
  ...
  return (
    <div className={recentFiles.length > 0 ? "desktop-start-screen" : "desktop-start-screen empty"} aria-label="Desktop start screen">
      <section className="desktop-start-main">
        ...
        <div className="desktop-start-actions" aria-label="Start actions">
          <button className="primary" type="button" onClick={onChooseWorkspaceDirectory} disabled={isWorkspaceLoading}>
            <FolderOpen size={18} />
      ...
      {recentFiles.length > 0 && <section className="desktop-start-recent" aria-label="Recent Documents">
          ...
          {recentFiles.map((entry) => (
            <li key={entry.id}>
              <button type="button" onClick={() => onOpenRecentFile(entry)}>
                ...
                <span>{formatRecentFileTime(entry.updatedAt)}</span>
