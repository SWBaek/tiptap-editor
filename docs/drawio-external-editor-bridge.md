...
The desktop layer should expose a narrow adapter, for example `DrawioExternalEditorBridge`, with these responsibilities:
...
- watch file save events and debounce duplicate writes;
...
- request preview regeneration when a preview renderer is available;
...
- Tauri command implementation and executable discovery.
