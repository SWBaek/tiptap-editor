...
| New document/folder, rename, trash delete/undo, refresh | Adapt | Partial. The selected explorer folder (or workspace root) can create a validated nested folder or a canonical packed `.sdoc`, and manual refresh remains available. Rename and trash delete/undo are not implemented yet. | Typed bridge/Rust commands reject absolute paths, traversal, symlink parents, invalid names, duplicate targets, and non-`.sdoc` files. Unit tests cover dialog/bridge/model validation; Rust tests cover filesystem guards; desktop-runtime E2E creates a nested folder and a valid `.sdoc`; desktop typecheck and packaged Tauri build pass. Rename and trash evidence remains required. |
