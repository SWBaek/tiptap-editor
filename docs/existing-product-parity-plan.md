...
| Save failure and external-change feedback | Adapt | Partial. Save failures now retain the dirty document/baseline and expose a durable Files recovery alert with the concrete error, Retry, and desktop Save As; only success clears the alert and advances the baseline/path. Watcher-driven current-document changes have a durable keep-current notice, while reload/compare choices still remain. | Unit tests prove native write errors propagate without false success. Desktop-runtime E2E proves failed writes retain Unsaved changes and expose Retry/Save As, then clear only after successful Retry. Full test/build/E2E, desktop typecheck, and Tauri build pass; external reload/compare evidence remains. |
