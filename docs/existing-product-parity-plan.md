...
| Editor-first fixed shell and isolated body scrolling | Adapt | Implemented. Shell, command bar, toolbar groups, panels, dialogs/inspectors, preview tabs, and runtime controls are separate typed components; `App.tsx` remains the shared state/command coordinator rather than a duplicated desktop frontend. | Editor remains primary at desktop/browser widths, component props retain behavior boundaries, and E2E covers panel, preview, toolbar-menu, dialog, and responsive flows. |
...
| Draw.io create/import and external edit | Adapt | Implemented with asset-backed source, contextual tools, typed desktop bridge, and explicit conflict actions. | E2E covers create/import/asset round trip and conflict revision; the packaged native checklist covers real external-editor launch/read-back. |
| New/Open/Save/Save As and deliverable Export | Adopt | Implemented; browser download and desktop native save-back routes are separated. | Browser E2E covers natural new-save-reopen-export flows; native open/save/recent continuity is prepared as an interactive smoke scenario rather than claimed as manually executed. |
| Desktop welcome/start screen and recent files | Adopt | Implemented. | Desktop-runtime E2E covers the entry boundary; native recent reopen is included in the prepared manual smoke checklist. |
...
| Save failure and external-change feedback | Adapt | Implemented. Save failures retain the dirty document/baseline and expose the concrete error, Retry, and desktop Save As. Watcher-driven current-document changes offer explicit Reload from disk, Keep current, and Compare actions; reload is never automatic and compare reads the disk version into a runtime-only semantic review baseline. | Unit tests prove native write errors propagate without false success. Desktop-runtime E2E proves failed writes retain Unsaved changes until Retry succeeds, Compare leaves the current draft untouched, Reload explicitly applies the disk version, and Keep current preserves dirty editor state. `npm test` (299), build, E2E (40), desktop typecheck, and packaged Tauri build pass. |
| Draw.io conflict feedback | Adopt | Implemented with a dedicated modal that names the diagram block, current source asset, and prospective revision asset. Keep current, Replace source, and Save as revision explain their persistence effects and execute without browser confirm prompts; conflict/session state remains runtime-only. | E2E proves no browser dialog appears, revision bytes are packed under `.sdoc/assets/`, only `sourceAssetId` changes, the diagram block ID remains stable, and no editor executable/source bytes/conflict state enters `document.json`. `npm test` (299), build, E2E (40), desktop typecheck, and packaged Tauri build pass. |
...
Gate prepared on 2026-07-18: all nine delivery slices have implementation and automated evidence, the packaged native smoke checklist is current, and the 3-5-user review kit is ready. Interactive native smoke and real-user results are intentionally not claimed; they are the next evidence to collect.
