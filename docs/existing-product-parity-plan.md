...
| Figure captions/list and table captions/list | Adopt | Implemented, including canonical table captions, Outline projections, and typed table caption editing. A coherent selected-image inspector remains in the separate image parity row. | Semantic/export/round-trip tests remain green; table caption authoring no longer uses a browser prompt. |
