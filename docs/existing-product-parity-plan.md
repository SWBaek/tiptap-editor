...
| Table context menu and inspector | Adapt | Partial. A selection-aware table context menu exposes caption and row/column actions; the caption flow still uses a prompt pending the technical-content dialog slice. | Context-menu E2E exercises a row mutation; the later inspector must add validated caption/header/alignment editing. |
| Equation and Mermaid hybrid editor with preview | Adapt | Partial. Inline/block equation insert and selected-node edit now share a validated KaTeX preview dialog; Mermaid insertion still uses `window.prompt` and has no inspector edit path. | Equation validation unit tests and E2E cover invalid-source blocking, preview, inline/block insert, stable-ID edit, export, and `.sdoc` round trip. Mermaid dialog/preview evidence remains required. |
