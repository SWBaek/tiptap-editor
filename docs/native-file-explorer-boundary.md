...
## Current Explorer Slice

The current desktop bridge exposes only the minimum workspace surface needed by the explorer-first Files panel:
...
- keep selected directory, listed entries, refresh state, clicked entry state, current-file highlighting, and recent-document paths as runtime/user state.
...
The current UI shows normal `.sdoc` files first. Developer unpack/pack commands are still available, but they are collapsed under a Developer workspace disclosure and are not part of the default authoring path.
