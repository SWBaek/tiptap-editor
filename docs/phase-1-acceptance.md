...
| Basic block editing | StarterKit toolbar supports heading, paragraph, bullet/ordered list, blockquote, and code block. `packages/editor-tiptap` converts editor JSON to SDoc JSON. Playwright verifies editor changes update the JSON preview. |
...
| Block ID lifecycle | `editor-tiptap` tests cover missing IDs, duplicate IDs, split blocks, list wrappers, nested blocks, and top-level block moves without ID loss. Playwright verifies split undo/redo and clipboard paste keep IDs unique. |
...
The browser smoke tests cover core happy/error paths, but they do not yet automate drag/drop or every toolbar command across mobile viewports.
