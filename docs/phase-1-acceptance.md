...
| Block ID lifecycle | `editor-tiptap` tests cover missing IDs, duplicate IDs, split blocks, list wrappers, nested blocks, and top-level block moves without ID loss. Playwright verifies split undo/redo keeps IDs unique. |
...
The browser smoke tests cover core happy/error paths, but they do not yet automate clipboard paste, drag/drop, or every toolbar command across mobile viewports.
