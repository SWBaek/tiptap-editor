...
| Area | Current State |
| --- | --- |
| Assets container | `.sdoc` already stores `assets/` in `packages/sdoc-format`, but the editor has no asset insertion or asset reference UI. |
| AI/RAG exports | `plain.md`, `chunks.jsonl`, `outline.json`, and `references.json` already exist in `packages/sdoc-export`. |
| Unsupported nodes | `figure`, `table`, `equationBlock`, and `diagram` are intentionally rejected by `packages/sdoc-schema` today. |
...
| --- | --- |
| Figure and image asset basics | Schema accepts `figure` with stable block ID, caption, and asset reference; `.sdoc` pack/open/save preserves the asset and reference. |
