...
| Unsupported nodes | `figure`, simple `table`, and inline/block equations are accepted Phase 2 slices. `diagram` is still intentionally rejected by `packages/sdoc-schema`. |
...
| Tests | Unit tests cover schema/export/format/diff behavior; Playwright covers figure, table, and equation browser workflows. Mermaid workflow remains. |
