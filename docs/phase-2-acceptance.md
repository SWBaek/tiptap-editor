...
| Area | Current State |
| --- | --- |
| Assets container | `.sdoc` stores `assets/` in `packages/sdoc-format`; figure references are checked against container assets. The editor still has no asset insertion UI. |
| AI/RAG exports | `plain.md`, `chunks.jsonl`, `outline.json`, and `references.json` already exist in `packages/sdoc-export`. |
| Unsupported nodes | `figure` is accepted as the first Phase 2 slice. `table`, `equationBlock`, and `diagram` are still intentionally rejected by `packages/sdoc-schema`. |
...
| --- | --- |
| Figure and image asset basics | Schema/export/format support `figure` with stable block ID, caption, and asset reference; editor insertion UI and browser round trip remain. |
| Simple table | Schema/editor support a minimal table with rows and cells; Markdown export emits a readable pipe table; diff identifies table block changes. |
...
| Tests | Unit tests cover schema/export/format behavior; Playwright covers at least one figure, table, equation, and Mermaid browser workflow. |
