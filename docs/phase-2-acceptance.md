...
| Unsupported nodes | Figure, simple table, inline/block equations, and Mermaid diagrams are accepted Phase 2 slices. Draw.io deep integration remains out of scope. |
| Mermaid | Mermaid source is stored as a typed `diagram` node and rendered as an SVG projection in the playground. |
...
| Equation basics | Covered for the MVP slice: inline `equation` and block `equationBlock` preserve `attrs.latex`, editor projection renders with KaTeX, Markdown export emits math delimiters, and browser `.sdoc` round trip is covered. |
| Mermaid basics | Covered for the MVP slice: `diagram` preserves `attrs.kind` and `attrs.source`, editor projection renders Mermaid SVG, Markdown export emits a fenced Mermaid block, and browser `.sdoc` round trip is covered. |
| Export upgrades | Markdown, chunks, outline, and references include figure/table captions where relevant. |
| Tests | Unit tests cover schema/export/format/diff behavior; Playwright covers figure, table, equation, and Mermaid browser workflows. |
