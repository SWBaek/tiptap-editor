...
- `docs/requirement-tagging-boundary.md` records the optional `attrs.humanId` policy, duplicate handling, export/RAG impact, and the rule that stable `attrs.id` remains the only internal identity key; unit tests cover validation, Tiptap preservation, semantic diff summaries, and derived export inclusion.
