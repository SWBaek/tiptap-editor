...
- the Review panel surfaces the latest batch result as runtime-only applied/skipped counts and per-failure messages so partially applied batches are explicit without storing review state in `document.json`.
