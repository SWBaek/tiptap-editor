...
Phase 5.3 corrects evidence gaps found after that acceptance. Slices 1-2 replace the fixed 320px column with a 260px default, 220-420px runtime-resizable sidebar; add `Ctrl+B`, `Ctrl+Shift+E`, and desktop `Ctrl+P`; make folder and file rows directly activate on one click; remove the non-interactive root decoration; move secondary actions under More; portal entry menus with complete interaction behavior; and add Quick Open, transient filtering, folders-first sort, runtime-only auto-reveal, sticky ancestors, and typed validated OS file-manager reveal. Compact folder chains and batch/drag interactions remain deferred; panel and responsive corrections are still open.
...
- A Draw.io external-edit conflict opens a dedicated modal that names the source asset and explains Keep current, Replace source, and Save as revision. The choice is explicit in the modal, requires no browser confirm prompt, and conflict/session state remains runtime-only.
- The packaged desktop webview uses an explicit production CSP: bundled scripts/styles/fonts are same-origin, inline style attributes remain allowed for editor geometry, in-memory `data:`/`blob:` images remain renderable, and network connections are limited to Tauri IPC. The `main` capability grants only native open/save dialogs; filesystem, workspace, watcher, trash, and Draw.io operations stay behind validated application commands rather than broad filesystem or shell plugin scopes.
- External-change recovery offers Reload from disk, Keep current editor state, and Compare. Reload is the only action that replaces editor state and does so by reopening the complete `.sdoc`; Keep dismisses the runtime conflict without changing canonical content; Compare parses the disk package into a runtime review base and opens semantic Review without changing the current document or saved baseline.
